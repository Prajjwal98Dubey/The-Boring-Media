      ...
      <div className="flex justify-center items-center">
        {isLoading ? (
          ...
          <div className="relative">
            <img src={LEFT_ARROW} className="w-[25px] h-[25px] absolute left-2 top-[40%] z-10 cursor-pointer" onClick={handlePrev} />
            <img src={RIGHT_ARROW} className="w-[25px] h-[25px] absolute right-2 top-[40%] z-10 cursor-pointer" onClick={handleNext} />
            ...
            <div className="flex absolute left-[40%] bottom-0">{new Array(userSuggestion.length).fill("").map((_,i)=>(
              <div key={i}>
                  <div className={`w-[10px] h-[10px] z-10 cursor-pointer rounded-full ${i===index ? 'bg-gray-200':'bg-gray-400'} m-1`} onClick={()=>setIndex(i)}></div>
              </div>
            )) }</div>
