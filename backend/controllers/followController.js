...
const handleFollowFollowing = async (req, res) => {
    ...
    try {
        ...
        const isRelationPresent = await Follow.findOne({ followingId: followingId, followerId: isFollowerUser._id })
...
const checkFollowing = async (req, res) => {
    ...
    try {
        const isUser = await User.findOne({ name: user })
        const isFollowing = await Follow.findOne({ followingId: loggedInUser, followerId: isUser._id })
        if (isFollowing) {
...
module.exports = { handleFollowFollowing, countFollowerAndFollowing, checkFollowing }